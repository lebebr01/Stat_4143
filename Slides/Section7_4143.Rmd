...
```{r normal, error=FALSE, warning=FALSE, message=FALSE, dev='png', fig.height=8, fig.width=12,background='white', echo=FALSE}
set.seed(2226)
temp <- data.frame(data = rnorm(100000))

library(ggplot2)

d <- ggplot(temp, aes(x = data)) + theme_bw(base_size=24)
d + geom_density() + scale_x_continuous("", breaks = seq(-3, 3, 1))
```
...
```{r normal2, error=FALSE, warning=FALSE, message=FALSE, dev='png', fig.height=8, fig.width=12,background='white', echo=FALSE}
set.seed(2226)
temp <- data.frame(data = rnorm(100000))

d <- ggplot(temp, aes(x = data)) + theme_bw(base_size=24)
d  + scale_y_continuous("Density", limits = c(0, .4)) + 
  geom_rect(aes(xmin = -1, xmax = 1, ymin = .1, ymax = .14), color = "lightgreen", fill = "lightgreen") + 
  geom_rect(aes(xmin = -2, xmax = 2, ymin = .06, ymax = .1), color = "lightblue", fill = "lightblue") +
  geom_rect(aes(xmin = -3, xmax = 3, ymin = .02, ymax = .06, color = "lightred", fill = "lightred")) + 
  theme(legend.position = "none") + geom_density() + 
  scale_x_continuous("", breaks = seq(-3, 3, 1)) +
  annotate("text", x = 0, y = .12, label = "68%", size = 7) + 
  annotate("text", x = 0, y = .08, label = "95%", size = 7) +
  annotate("text", x = 0, y = .04, label = "99.7%", size = 7) 
```

# The Empirical Rule 2
```{r normal3, error=FALSE, warning=FALSE, message=FALSE, dev='png', fig.height=8, fig.width=12,background='white', echo=FALSE}
set.seed(2226)
temp <- data.frame(data = rnorm(100000))
...
d  + scale_y_continuous("Density", limits = c(0, .4)) + 
  ...
  geom_rect(aes(xmin = -2, xmax = 2, ymin = .06, ymax = .1), color = "lightblue", fill = "lightblue") +
  geom_rect(aes(xmin = -3, xmax = 3, ymin = .02, ymax = .06, color = "lightred", fill = "lightred")) + 
  geom_vline(xintercept = 0, lwd = 1) + geom_density() + scale_x_continuous("", breaks = seq(-3, 3, 1)) + theme(legend.position = "none") + 
  annotate("text", x = -.5, y = .12, label = "34%", size = 7) + 
  annotate("text", x = .5, y = .12, label = "34%", size = 7) +
  annotate("text", x = -1, y = .08, label = "47.5%", size = 7) + 
  annotate("text", x = 1, y = .08, label = "47.5%", size = 7) +
  annotate("text", x = -1.25, y = .04, label = "49.85%", size = 7) + 
  annotate("text", x = 1.25, y = .04, label = "49.85%", size = 7)
```
...
```{r ztab, error=FALSE, warning=FALSE, message=FALSE, dev='png', fig.height=8, fig.width=12,background='white', echo=FALSE}
set.seed(2226)
temp <- data.frame(data = rnorm(100000))

d <- ggplot(temp, aes(x = data)) + theme_bw(base_size=24)
d  + scale_y_continuous("Density", limits = c(0, .4)) + 
  geom_rect(aes(xmin = 0, xmax = 1, ymin = .02, ymax = .06), color = "lightblue", fill = "lightblue") +
  geom_rect(aes(xmin = 1, xmax = 4, ymin = .02, ymax = .06, color = "lightred", fill = "lightred")) + 
  theme(legend.position = "none") + geom_density() + 
  geom_vline(xintercept = 0, size = 1) + 
  geom_vline(xintercept = 1, size = 1, linetype = 2) + 
  scale_x_continuous("", breaks = seq(-4, 4, 1)) +
  annotate("text", x = .4, y = .04, label = ".3413", size = 7) +
  annotate("text", x = 2, y = .04, label = ".1587", size = 7) 
```
